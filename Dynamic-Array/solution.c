...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    int* seqCapacity = calloc(n, sizeof(int)); // track capacity for realloc
    ...
    int lastAnswer = 0;
    int* results = malloc(queries_rows * sizeof(int)); // worst case all queries are type 2
    int resIndex = 0;
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (type == 1) {
            ...
            if (seqSizes[idx] == seqCapacity[idx]) {
                seqCapacity[idx] = seqCapacity[idx] == 0 ? 1 : seqCapacity[idx] * 2;
